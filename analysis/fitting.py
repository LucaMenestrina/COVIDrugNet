...
def onesample(data):
    ...
    upper_sample = getattr(fitted, "power_law").generate_random(
        n=len(data) - len(lower)
    )
...
def fittings_plot(data, folder=""):
    ...
    for dist in [
        ...
        plt.plot(
            reducedx,
            getattr(fitted, dist).pdf(reducedx)
            * len(data[data >= fitted.xmin])
            / len(data),
...
def estimate(data, parms, percentile, samples, title):
    ...
    for func in [onesample.remote, naivesample.remote]:
        ...
        replicates_collection = {
            parm: [l[parm] for l in replicates_collection] for parm in parm_to_bootstrap
        }
        for parm in parm_to_bootstrap:
            ...
            axs[pos, 1].plot(
                [np.std(replicates[: i + 2]) for i in range(len(replicates))]
            )
            ...
            axs[pos, 2].plot(
                [
                    ...
                    for i, n in enumerate(
                        np.cumsum(
                            [
                                len([n for n in replicates[:ind] if n >= parms[parm]])
                                / len(replicates[:ind])
                                for ind in range(1, len(replicates))
                            ]
                        )
                    )
                ]
            ...
            axs[pos, 3].hist(
                replicates,
                bins=int(len(set([round(n, significant_figures) for n in replicates]))),
                density=True,
            )
            sns.kdeplot(replicates, ax=axs[pos, 3], color="lightblue", label="KDE")
            axs[pos, 3].axvspan(
                0,
                np.percentile(np.sort(replicates), 90),
                alpha=0.15,
                color="green",
            ...
            axs[pos, 3].axvline(
                np.percentile(np.sort(replicates), 90), color="green", linestyle="--"
            )  # , label="0.10 p value acceptance threshold"
            ...
            axs[pos, 3].set_xlim(
                minmax(replicates)
            )  # CI[0]-3*std, CI[1]+3*std #[min(replicates),max(replicates)]
...
def test_distribution(data, title, percentile=0.90, samples=1e5):
    ...
    comparisons = {
        alternative: dict(
            zip(
                ...
                results.distribution_compare("power_law", alternative),
            )
        )
        for alternative in [
            "truncated_power_law",
    ...
    print(
        "Test runtime %sh %sm %ss"
        % tuple(str(timedelta(seconds=(time() - start))).split(":"))
    )
...
proteins_to_remove = [
    node
    for node in dt.neighbors("Artenimol (DB11638)")
    if (
        list(dt.neighbors(node)) == ["Artenimol (DB11638)"]
        or list(dt.neighbors(node)) == ["Artenimol (DB11638)", "Fostamatinib (DB12010)"]
    )
] + [
    node
    for node in dt.neighbors("Fostamatinib (DB12010)")
    if (
        list(dt.neighbors(node)) == ["Fostamatinib (DB12010)"]
        or list(dt.neighbors(node)) == ["Artenimol (DB11638)", "Fostamatinib (DB12010)"]
    )
]
dt_removed = dt.copy()
dt_removed.remove_nodes_from(
    ["Artenimol (DB11638)", "Fostamatinib (DB12010)"] + proteins_to_remove
)
drugs = [
    node
    for node in dt_removed
    if (
        dt_removed.nodes()[node]["kind"] == "Drug"
        and dt_removed.nodes()[node]["Targets"] != []
    )
]
