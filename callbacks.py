...
def show_edges_callback(prefix, nodes, edges, edges_to_show):
    ...
    def show_edges(show_all):
        if (
            prefix != "target_projection"
        ):  # useless because it will never be fired outside the target projection page
...
def displayHoverNodeData_callback(prefix, G):
    ...
    def displayHoverNodeData(data, selected_data, warning):
        ...
        if not data:
            data = {
                "Name": "",
                "Properties": "Hover over a node (or select it) to show its properties",
            }
                ...
                if data["ID"] != "Not Available":
                    img = html.A(
                        html.Img(
                            ...
                            height="auto",
                            width="100%",
                            alt="Structure Image not Available",
                        ),
                        href="https://www.drugbank.ca/structures/small_molecule_drugs/"
                        + data["ID"],
                        target="_blank",
                    )
            else:
                ...
                if data["PDBID"] != "Not Available":
                    img = [
                        html.A(
                            ...
                            href="https://www.rcsb.org/3d-view/" + data["PDBID"],
                            target="_blank",
                        ),
                        html.A(
                            html.Small(
                            ...
                            href="http://doi.org/10.2210/pdb%s/pdb" % data["PDBID"],
                            target="_blank",
                        ),
                    ]
        attributes_list = []
        for attribute in attributes:
            if data[attribute] != "" and data[attribute] != []:
                if attribute in [
                    ...
                    attributes_list.append(
                        html.Li(
                            [
                                html.Strong(attribute + ": "),
                                html.A(
                                    data[attribute], href=link_drugbank, target="_blank"
                                ),
                            ],
                            className="list-group-item",
                        )
                    ...
                    if not (
                        len(data["Trials"]) == 1
                        and data["Trials"][0] == "Not Available"
                    ):
                        trials_list = [html.Strong("Trials: ")]
                        for n, trial in enumerate(data["Trials"]):
                            trials_list.append(
                                html.A(
                                    trial, href=data["Trials Hrefs"][n], target="_blank"
                                )
                            )
                        ...
                        attributes_list.append(
                            dbc.Container(
                                html.Li(trials_list, className="list-group-item"),
                                style={
                                    "max-height": "20vh",
                                    "overflow-y": "auto",
                                    "padding": "0",
                    ...
                    attributes_list.append(
                        html.Li(
                            [html.Strong(attribute + ": "), data[attribute]],
                            className="list-group-item",
                        )
                    )
...
def inspected_table_callback(prefix):
    ...
    def inspected_table(selected_data, highlighted_data, to_inspect):
        results = [
            html.H5("No Data to Show"),
            html.P("(Try changing the value of the dropdown menu)"),
        ]
        open = False
        ...
        if to_inspect == "selected":
            ...
            data = [
                merged[id]
                for id in set(selected.keys()).intersection(set(highlighted.keys()))
            ]
        if data:
            if len(data) >= 1 and open:
                ...
                if len(drugs_data) > 0:
                    ...
                    table_header = [
                        html.Thead(
                            html.Tr(
                                [
                                    html.Th(attribute, style={"white-space": "nowrap"})
                                    for attribute in [
                                        "Name",
                                        "ID",
                                        "ATC Level 1",
                            ...
                            if attribute == "Name":
                                ...
                                row.append(
                                    html.Td(
                                        html.A(
                                            drug["ID"],
                                            href="https://www.drugbank.ca/drugs/"
                                            + drug["ID"],
                                            target="_blank",
                                        )
                                    )
                    ...
                    drugs_table = dbc.Table(
                        table_header + table_body,
                        className="table table-hover",
                        bordered=True,
                    )
                    drugs_href = "data:text/csv;charset=utf-8," + quote(
                        pd.DataFrame(
                            [
                                {
                                    key: (
                            ...
                            columns=attributes,
                        ).to_csv(sep="\t", index=False, encoding="utf-8")
                    )
                    results += [
                        html.Br(),
                        dbc.Row(
                                        ...
                                        [
                                            ...
                                            dbc.Col(
                                                html.A(
                                                    ...
                                                    id=prefix
                                                    + "_inspected_table_drugs_glossary",
                                                    href="/covidrugnet/help#glossary",
                                                    target="_blank",
                                                ),
                                                align="center",
                                            ),
                ...
                if len(targets_data) > 0:
                    ...
                    for target in targets_data:
                        ...
                        for attribute in attributes:
                            if attribute == "Name":
                                row.append(
                                    html.Td(
                                        target["Name"],
                                        style={"min-width": "8vw", "max-width": "15vw"},
                                    )
                                )
                            elif attribute == "ID":
                                row.append(
                        ...
                        table_body.append(html.Tr(row))
                    table_body = [html.Tbody(table_body)]
                    targets_table = dbc.Table(
                        table_header + table_body,
                        className="table table-hover",
                        bordered=True,
                    )
                    targets_href = "data:text/csv;charset=utf-8," + quote(
                        pd.DataFrame(
                            [
                                {
                                    key: (
                                    ...
                                    dbc.Row(
                                        [
                                            html.A(
                                                ...
                                                href=targets_href,
                                                download="inspected_targets.tsv",
                                                target="_blank",
                                            ),
                                            dbc.Col(
                                                html.A(
                                                    ...
                                                    id=prefix
                                                    + "_inspected_table_targets_glossary",
                                                    href="/covidrugnet/help#glossary",
                                                    target="_blank",
                                                ),
                                                align="center",
                ...
                return (
                    dbc.Container(results, fluid=True),
                    True,
                    False,
                    visibility,
                    open,
                ...
                return (
                    dbc.Container(results, fluid=True, style={"padding": "3%"}),
                    False,
                    True,
                    visibility,
                    open,
...
def properties_table_callback(prefix, graph_properties_df, nodes):
    ...
    def properties_table(search, sorting, rows, only_inspected, inspected_data):
        ...
        if prefix == "drug_target":
            df.drop(
                "Clustering Coefficient", inplace=True, axis=1
            )  # removes the clustering coefficient column from the dataframe of the Drug-Target Networks, since it is bipartited and the clustering coefficient has no meaning in this case
        if only_inspected and inspected_data:
            df = df.loc[[node["nameID"] for node in inspected_data]]
            options = [
                {"label": name, "value": name}
                for name in [node["nameID"] for node in inspected_data]
            ]
        else:
            options = [
                {"label": data["nameID"], "value": data["nameID"]}
                for data in [node["data"] for node in nodes]
            ]
        if search:
            df = df.loc[search]
        ...
        href = "data:text/csv;charset=utf-8," + quote(
        ...
        return (
            dbc.Table.from_dataframe(
                ...
                bordered=True,
                className="table table-hover",
                id=prefix + "_graph_properties_table",
            ),
            href,
            options,
        )
...
def group_highlighter_callback(prefix, nodes, maj):
    ...
    @app.callback(
        ...
        [Input(prefix + "_only_major_highlighted_groups", "value")]
        + [
            Input(prefix + "_highlight_dropdown_" + property, "value")
            for property in properties
        ]
        + [
            Input(prefix + "_conjunction_" + property, "value")
            for property in properties
        ]
        + [
            Input(prefix + "_equality_" + centrality, "value")
            for centrality in centralities
        ]
        + [
            Input(prefix + "_highlight_value_" + centrality, "value")
            for centrality in centralities
        ]
        + [Input(prefix + "_conjunction_general", "value")],
    )
    def group_highlighter(only_maj, *values):
        if (
            isinstance(only_maj, list) and len(only_maj) > 0 and only_maj[0] == True
        ):  # could be avoided using daq BooleanSwitch
            only_maj = True
        else:
            only_maj = False
        highlighted_values = dict(
            zip(
                ["highlighted_" + property for property in properties]
                + ["conjunction_" + property for property in properties]
                + ["equality_" + centrality for centrality in centralities]
                + ["value_" + centrality for centrality in centralities]
                + ["conjunction_general"],
                values,
            )
        )
        for centrality in centralities:
            highlighted_values["highlighted_" + centrality] = centrality_filter(
                centrality,
                highlighted_values["equality_" + centrality],
                highlighted_values["value_" + centrality],
            )
        highlighted = unite(
            highlighted_values["conjunction_general"],
            [
                unite(
                    highlighted_values["conjunction_" + property],
                    highlighted_values["highlighted_" + property],
                )
                for property in properties
            ]
            + [
                highlighted_values["highlighted_" + centrality]
                for centrality in centralities
            ],
        )
        if highlighted:
            ...
            href = "data:text/csv;charset=utf-8," + quote(
                pd.DataFrame(
                    [
                        {
                            attribute: (
                                ", ".join(node["data"][attribute])
                                if isinstance(node["data"][attribute], list)
                                else node["data"][attribute]
                            )
                            for attribute in attributes
                        }
                        for node in nodes
                        if node["data"]["ID"] in highlighted
                    ],
                    columns=attributes,
                ).to_csv(sep="\t", index=False, encoding="utf-8")
            )
    ...
    @app.callback(
        [Output(prefix + "_only_major_highlighted_groups", "value")]
        + [
            Output(prefix + "_highlight_dropdown_" + property, "value")
            for property in properties
        ]
        + [
            Output(prefix + "_conjunction_" + property, "value")
            for property in properties
        ]
        + [
            Output(prefix + "_equality_" + centrality, "value")
            for centrality in centralities
        ]
        + [
            Output(prefix + "_highlight_value_" + centrality, "value")
            for centrality in centralities
        ]
        + [Output(prefix + "_conjunction_general", "value")],
    ...
    def clear_group_highlighter(n):
        return (
            [True]
            + [None for property in properties]
            + ["OR" for property in properties]
            + [">=" for centrality in centralities]
            + [None for centrality in centralities]
            + ["OR"]
        )
...
def highlighter_callback(
    ...
    cmap = dict(
        zip(
            sorted(set(d_comp.values())),
            [
                rgb2hex(plt.cm.Spectral(i))
                for i in np.arange(0, 1.00001, 1 / len(components))
            ],
        )
    )
    id2component = {
        name2id[node]: cmap.get(d_comp[node], "#708090") for node in G.nodes()
    }
    stylesheet_components = []
    for ID in id2component:
        stylesheet_components.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
                    "border-width": 2,
                    "background-color": id2component[ID],
                },
            }
        )
    pie_data = go.Pie(
        labels=list(range(1, len(components) + 1)),
        values=[len(component) for component in components],
        marker_colors=[
            rgb2hex(plt.cm.Spectral(i))
            for i in np.arange(0, 1.00001, 1 / len(components))
        ],
    )
    pie_components = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Components' Node Distribution",
                "x": 0.5,
                "xanchor": "center",
            }
        },
    )
    pie_components.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Component: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    ...
    cmap = dict(
        zip(
            ...
            [
                rgb2hex(plt.cm.Spectral(i))
                for i in np.arange(0, 1.00001, 1 / n_clusters)
            ],
        )
    )
    ...
    for ID in id_cluster:
        stylesheet_spectral.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
                    "border-width": 2,
                    "background-color": cmap[id_cluster[ID]],
                },
            }
        )
    pie_data = go.Pie(
        labels=list(range(1, len(clusters) + 1)),
        values=[list(clusters).count(cluster) for cluster in range(len(set(clusters)))],
        marker_colors=[
            rgb2hex(plt.cm.Spectral(i))
            for i in np.arange(0, 1.00001, 1 / len(set(clusters)))
        ],
    )
    pie_spectral = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Clusters' Node Distribution",
                "x": 0.5,
                "xanchor": "center",
            }
        },
    )
    pie_spectral.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Cluster: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_spectral = html.P(
        [
            "Nodes are colored on the corresponding cluster, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
            html.A("plots sections", href="#" + prefix + "_plots"),
            " for more info",
        ]
    )
    # spectral_maj
    cmap = dict(
        zip(
            sorted(set(clusters_maj)),
            [
                rgb2hex(plt.cm.Spectral(i))
                for i in np.arange(0, 1.00001, 1 / n_clusters_maj)
            ],
        )
    )
    id_cluster = dict(
        zip(dict(nx.get_node_attributes(maj, "ID")).values(), clusters_maj)
    )
    stylesheet_spectral_maj = []
    for ID in id_cluster:
        stylesheet_spectral_maj.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
                    "border-width": 2,
                    "background-color": cmap[id_cluster.get(ID, "#708090")],
                },
            }
        )
    pie_data = go.Pie(
        labels=list(range(1, len(clusters_maj) + 1)),
        values=[
            list(clusters_maj).count(cluster)
            for cluster in range(len(set(clusters_maj)))
        ],
        marker_colors=[
            rgb2hex(plt.cm.Spectral(i))
            for i in np.arange(0, 1.00001, 1 / len(set(clusters_maj)))
        ],
    )
    pie_spectral_maj = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Clusters' Node Distribution",
    ...
    pie_spectral_maj.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Cluster: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_spectral_maj = html.P(
        [
            "Nodes are colored on the corresponding cluster, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
    ...
    cmap = dict(
        zip(
            sorted(set(d_comm.values())),
            [
                rgb2hex(plt.cm.Spectral(i))
                for i in np.arange(0, 1.00001, 1 / len(communities))
            ],
        )
    )
    id2community = {
        name2id[node]: cmap.get(d_comm[node], "#708090") for node in G.nodes()
    }
    stylesheet_girvan_newman = []
    for ID in id2community:
        stylesheet_girvan_newman.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
                    "border-width": 2,
                    "background-color": id2community[ID],
                },
            }
        )
    pie_data = go.Pie(
        labels=list(range(1, len(communities) + 1)),
        values=[len(community) for community in communities],
        marker_colors=[
            rgb2hex(plt.cm.Spectral(i))
            for i in np.arange(0, 1.00001, 1 / len(communities))
        ],
    )
    pie_girvan_newman = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Communities' Node Distribution",
                "x": 0.5,
                "xanchor": "center",
            }
        },
    )
    pie_girvan_newman.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Community: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_girvan_newman = html.P(
        [
            "Nodes are colored on the corresponding community, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
            html.A("plots sections", href="#" + prefix + "_plots"),
            " for more info",
    ...
    id2community = {
        name2id[node]: cmap.get(d_comm[node], "#708090") for node in maj.nodes()
    }
    stylesheet_girvan_newman_maj = []
    for ID in id2community:
        stylesheet_girvan_newman_maj.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
                    "border-width": 2,
                    "background-color": id2community.get(ID, "#708090"),
                },
            }
        )
    pie_data = go.Pie(
    ...
    pie_girvan_newman_maj = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Communities' Node Distribution",
    ...
    pie_girvan_newman_maj.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Community: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_girvan_newman_maj = html.P(
        [
            "Nodes are colored on the corresponding community, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
    ...
    for ID in id2community:
        stylesheet_greedy_modularity.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
    ...
    pie_greedy_modularity = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Communities' Node Distribution",
    ...
    pie_greedy_modularity.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Community: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_greedy_modularity = html.P(
        [
            "Nodes are colored on the corresponding community, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
    ...
    for ID in id2community:
        stylesheet_greedy_modularity_maj.append(
            {
                "selector": "[ID = '" + ID + "']",
                "style": {
                    "border-color": "#303633",
    ...
    pie_greedy_modularity_maj = go.Figure(
        data=pie_data,
        layout={
            "title": {
                "text": "Communities' Node Distribution",
    ...
    pie_greedy_modularity_maj.update_traces(
        textposition="inside",
        textinfo="label+percent",
        hovertemplate=" Community: %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
    )
    legend_body_greedy_modularity_maj = html.P(
        [
            "Nodes are colored on the corresponding community, check the ",
            html.A("clustering", href="#" + prefix + "_clustering"),
            " or ",
    ...
    def highlighter(
        ...
        if changed_id != prefix + "_highlighter_dropdown.value":
            if "Centrality" in coloring or coloring in [
                name + comp
                for name in ["Degree", "Clustering Coefficient", "VoteRank Score"]
                for comp in ["", "_maj"]
            ]:
                if "maj" in coloring:
                    ...
                    centrality = dict(
                        zip(
                            nx.get_node_attributes(maj, "ID").values(),
                            nx.get_node_attributes(maj, coloring).values(),
                        )
                    )
                else:
                    centrality = {
                        node["data"]["ID"]: node["data"][coloring] for node in nodes
                    }
                values_unique = sorted(set(centrality.values()))
                scaled = sorted(
                    MinMaxScaler()
                    .fit_transform(np.array(values_unique).reshape(-1, 1))
                    .reshape(1, -1)[0]
                    .tolist()
                )
                cmap = dict(
                    zip(values_unique, [rgb2hex(plt.cm.coolwarm(i)) for i in scaled])
                )
                stylesheet = []
                    ...
                    stylesheet.append(
                        {
                            ...
                            "style": {
                                "border-color": "#303633",
                                "border-width": 2,
                                "background-color": cmap.get(c, "#708090"),
                            },
                        }
                    )
                ...
                bins = (
                    binner.fit_transform(np.array(values).reshape(-1, 1))
                    .reshape(1, -1)[0]
                    .tolist()
                )
                ...
                if "Degree" not in coloring:
                    bin_edges = [
                        (
                            round(binner.bin_edges_[0][i], 3),
                            round(binner.bin_edges_[0][i + 1], 3),
                        )
                        for i in range(nbins)
                    ]
                    labels = np.array(
                        [
                            "%.3f \u2264 %s \u2264 %.3f" % (edge[0], coloring, edge[1])
                            for edge in bin_edges
                        ]
                    )[[int(n) for n in set(bins)]]
                else:
                    bin_edges = [
                        (int(binner.bin_edges_[0][i]), int(binner.bin_edges_[0][i + 1]))
                        for i in range(nbins)
                    ]
                    labels = np.array(
                        [
                            "%d \u2264 %s \u2264 %d"
                            % (int(edge[0]), coloring, int(edge[1]))
                            for edge in bin_edges
                        ]
                    )[[int(n) for n in set(bins)]]
                mid_points = np.array(
                    [(edge[0] + edge[1]) / 2 for edge in bin_edges]
                ).reshape(-1, 1)
                marker_colors = np.array(
                    [
                        rgb2hex(plt.cm.coolwarm(i))
                        for i in sorted(
                            MinMaxScaler()
                            .fit_transform(mid_points)
                            .reshape(1, -1)[0]
                            .tolist()
                        )
                    ]
                ...
                pie_data = go.Pie(
                    labels=labels, values=counts, marker_colors=marker_colors
                )
                pie = go.Figure(
                    data=pie_data,
                    layout={
                        "title": {
                            "text": "Nodes' Centrality Distribution",
                            "x": 0.5,
                            "xanchor": "center",
                        }
                    },
                ...
                pie.update_traces(
                    textposition="inside",
                    textinfo="label+percent",
                    hovertemplate=" %{label} <br> Nodes: %{value} </br> %{percent} <extra></extra>",
                )
                colors = np.array(
                    [rgb2hex(plt.cm.coolwarm(i)) for i in np.linspace(0, 1, 10)]
                )
                legend_palette = px.imshow(
                    np.linspace(min(values), max(values), 1000).reshape(-1, 1),
                    color_continuous_scale=colors,
                    y=np.linspace(min(values), max(values), 1000),
                    labels={"y": coloring},
                    aspect="auto",
                )
                legend_palette.update(layout_coloraxis_showscale=False)
                legend_palette.update_xaxes(showticklabels=False)
                legend_palette.update_traces(
                    hovertemplate=" " + coloring + ": %{y:.3f} <extra></extra>"
                )
                legend_palette.update_layout(margin={"pad": 0, "t": 5, "b": 5})
                legend_body = dcc.Graph(
                    figure=legend_palette,
                    responsive=True,
            ...
            if coloring == "categorical":
                ...
                pie_data = go.Pie(
                    labels=["Drugs", "Targets"],
                    values=[
                        len([node for node, kind in G.nodes("kind") if kind == "Drug"]),
                        len(
                            [node for node, kind in G.nodes("kind") if kind == "Target"]
                        ),
                    ],
                    marker_colors=["#FC5F67", "#12EAEA"],
                )
                pie = go.Figure(
                    data=pie_data,
                    layout={
                        "title": {
                            "text": "Nodes' Categories Distribution",
                            "x": 0.5,
                            "xanchor": "center",
                        }
                    },
                ...
                legend_body = dbc.Table(
                    html.Tbody(
                        [
                            html.Tr(
                                [
                                    html.Td("", style={"background-color": "#FC5F67"}),
                                    html.Td("Drugs"),
                                ]
                            ),
                            html.Tr(
                                [
                                    html.Td("", style={"background-color": "#12EAEA"}),
                                    html.Td("Targets"),
                                ]
                            ),
                        ]
                ...
                target_classes = {
                    node["data"]["nameID"]: node["data"]["Target Class"]
                    for node in nodes
                }
                all_classes = sorted(
                    set([l for ll in target_classes.values() for l in ll])
                )
                cmap = dict(
                    zip(
                        all_classes,
                        [
                            rgb2hex(plt.cm.Spectral(n))
                            for n in np.arange(0, 1.1, 1 / len(all_classes))
                        ],
                    )
                )
                cmap.update({"Not Available": "#708090"})
                class2num = {
                    list(cmap.keys())[num]: str(num + 1)
                    for num in range(len(all_classes))
                }
                stylesheets = []
                classes_count = {}
                tot_classes = 0
                for node, classes in target_classes.items():
                    ...
                    for target_class in classes:
                        style.update(
                            {
                                "pie-"
                                + class2num[target_class]
                                + "-background-color": cmap[target_class],
                                "pie-"
                                + class2num[target_class]
                                + "-background-size": (
                                    100 * classes.count(target_class)
                                )
                                / len(classes),
                            }
                        )
                ...
                pie_data = go.Pie(
                    labels=list(classes_count.keys()),
                    values=[int(value) for value in classes_count.values()],
                    marker_colors=[
                        cmap[target_class] for target_class in classes_count.keys()
                    ],
                )
                pie = go.Figure(
                    data=pie_data,
                ...
                for target_class in cmap:
                    table_body.append(
                        html.Tr(
                            [
                                html.Td(
                                    "", style={"background-color": cmap[target_class]}
                                ),
                                html.Td(target_class),
                            ]
                        )
                    )
                legend_body = dbc.Table(
                    html.Tbody(table_body), borderless=True, size="sm"
                )

            elif coloring == "trialphase":
                trial_phases = {
                    node["data"]["nameID"]: node["data"]["Trials Phases"]
                    for node in nodes
                }
                all_phases = sorted(
                    set([l for ll in trial_phases.values() for l in ll])
                )
                cmap = dict(
                    zip(
                        all_phases,
                        [
                            rgb2hex(plt.cm.RdYlGn(n))
                            for n in np.arange(0, 1.1, 1 / len(all_phases))
                        ],
                    )
                )
                cmap.update({"Not Available": "#708090"})
                phase2num = {
                    list(cmap.keys())[num]: str(num + 1)
                    for num in range(len(all_phases))
                }
                stylesheets = []
                phases_count = {}
                tot_phases = 0
                for node, phases in trial_phases.items():
                    ...
                    for trial_phase in phases:
                        style.update(
                            {
                                "pie-"
                                + phase2num[trial_phase]
                                + "-background-color": cmap[trial_phase],
                                "pie-"
                                + phase2num[trial_phase]
                                + "-background-size": (100 * phases.count(trial_phase))
                                / len(phases),
                            }
                        )
                        try:
                ...
                pie_data = go.Pie(
                    labels=[f"Phase: {p}" for p in phases_count.keys()],
                    values=[int(value) for value in phases_count.values()],
                    marker_colors=[
                        cmap[trial_phase] for trial_phase in phases_count.keys()
                    ],
                )
                pie = go.Figure(
                    data=pie_data,
                ...
                for trial_phase in cmap:
                    table_body.append(
                        html.Tr(
                            [
                                html.Td(
                                    "", style={"background-color": cmap[trial_phase]}
                                ),
                                html.Td(f"Phase: {trial_phase}"),
                            ]
                        )
                    )
                legend_body = dbc.Table(
                ...
                for node in nodes:
                    ...
                    for atc in node["data"]["ATC Code Level 1"]:
                        style.update(
                            {
                                "pie-" + atc2num[atc] + "-background-color": cmap[atc],
                                "pie-"
                                + atc2num[atc]
                                + "-background-size": 100
                                / len(node["data"]["ATC Code Level 1"]),
                            }
                        )
                    stylesheet.update({"style": style})
                    stylesheets.append(stylesheet)
                ...
                ATC_count = {
                    k: v
                    for k, v in sorted(
                        ATC_count.items(), key=lambda item: item[1], reverse=True
                    )
                }  # sort by value
                ...
                ATC3_count = {
                    atc: tmp_atc3_values.count(atc) for atc in set(tmp_atc3_values)
                }
                ATC3_count = {
                    k: v
                    for k, v in sorted(
                        ATC3_count.items(), key=lambda item: item[1], reverse=True
                    )
                }  # sort by value
                ATC1_getter = {
                    atc3: atc3[0] if atc3 != "Not Available" else "Not Available"
                    for atc3 in ATC3_count.keys()
                }
                ...
                for code, value in ATC3_count.items():
                    if ATC1_getter[code] in ATC1_apparent_count:
                        ...
                        ATC1_apparent_count[ATC1_getter[code]] = value
                ATC3_count_normalized = {
                    k: v
                    * ATC_count[ATC1_getter[k]]
                    / ATC1_apparent_count[ATC1_getter[k]]
                    for k, v in ATC3_count.items()
                }
                ...
                for atc3 in ATC3_count:
                    ...
                    pie_data.append(
                        go.Bar(
                            ...
                            y=y,
                            marker={
                                "color": cmap[ATC1_getter[atc3]],
                                "line": {"color": "lightgrey", "width": 1},
                            },
                            text=text,
                            meta=[
                                ATC3_count[atc3],
                                long_atc[ATC1_getter[atc3]],
                                ATC_count[ATC1_getter[atc3]],
                            ],
                        )
                    )
                pie = go.Figure(
                    ...
                    layout={
                        ...
                        "yaxis": {"title_text": "Number of Nodes"},
                    },
                )
                pie.update_traces(
                    hovertemplate="<b> ATC Level 3 </b> <br> %{text} <br> Nodes: %{meta[0]} <br><br><b> ATC Level 1 </b> <br> %{meta[1]}  <br> Nodes: %{meta[2]} <extra></extra>"
                )
                ...
                for code in cmap:
                    table_body.append(
                        html.Tr(
                            [
                                html.Td("", style={"background-color": cmap[code]}),
                                html.Td(long_atc[code]),
                            ]
                        )
                    )
                legend_body = dbc.Table(
                ...
                property_name = {
                    ...
                    "location": "Cellular Location",
                }[coloring]
                all_properties = sorted(
                    set([node["data"][property_name] for node in nodes])
                )
                cmap = dict(
                    zip(
                        all_properties,
                        [
                            rgb2hex(plt.cm.Spectral(n))
                            for n in np.arange(0, 1.1, 1 / len(all_properties))
                        ],
                    )
                )
                cmap.update({"Not Available": "#708090"})
                ...
                for node in nodes:
                    stylesheet = {"selector": "[ID = '" + node["data"]["ID"] + "']"}
                    style = {
                        "background-color": cmap[node["data"][property_name]],
                        "border-color": "#303633",
                        "border-width": 2,
                    }
                    stylesheet.update({"style": style})
                ...
                properties_count = {
                    property: list(properties_dict.values()).count(property)
                ...
                pie_data = go.Pie(
                    ...
                    values=list(properties_count.values()),
                    marker_colors=[cmap[property] for property in all_properties],
                )
                pie = go.Figure(
                    data=pie_data,
                    layout={
                ...
                for property in all_properties:
                    table_body.append(
                        html.Tr(
                            [
                                html.Td("", style={"background-color": cmap[property]}),
                                html.Td(property),
                            ]
                        )
                    )
                ...
                if custom_component == "maj":
                    ...
                    greedy_modularity_custom_stylesheet = (
                        stylesheet_greedy_modularity_maj.copy()
                    )
                    ...
                    greedy_modularity_custom_stylesheet = (
                        stylesheet_greedy_modularity.copy()
                    )
                ...
                if custom_method == "spectral_group":
                    ...
                    cmap_custom = dict(
                        zip(
                            sorted(set(clusters_custom)),
                            [
                                rgb2hex(plt.cm.Spectral(i))
                                for i in np.arange(0, 1.00001, 1 / custom_n)
                            ],
                        )
                    )
                    id_cluster_custom = dict(
                        zip(
                            dict(nx.get_node_attributes(graph, "ID")).values(),
                            clusters_custom,
                        )
                    )
                    stylesheet = []
                        ...
                        stylesheet.append(
                            {
                                ...
                                "style": {
                                    "border-color": "#303633",
                                    "border-width": 2,
                                    "background-color": cmap_custom[
                                        id_cluster_custom[ID]
                                    ],
                                },
                            }
                        )
                    pie_data = go.Pie(
                        labels=list(range(1, len(clusters_custom) + 1)),
                        values=[
                            list(clusters_custom).count(cluster)
                            for cluster in range(len(set(clusters_custom)))
                        ],
                        marker_colors=[
                            rgb2hex(plt.cm.Spectral(i))
                            for i in np.arange(
                                0, 1.00001, 1 / len(set(clusters_custom))
                            )
                        ],
                    )
                    pie = go.Figure(
                    ...
                    id2community = {
                        name2id[node]: cmap.get(d_comm[node], "#708090")
                ...
                legend_body = html.P(
                    [
                        "Nodes are colored on the corresponding cluster/community, check the ",
                        html.A("clustering", href="#" + prefix + "_clustering"),
                        " or ",
                        html.A("plots sections", href="#" + prefix + "_plots"),
                        " for more info",
            ...
            pie.update_layout(
                ...
                showlegend=False,
                template="ggplot2",
            )  # title={"text":"Groups' Node Distribution","x":0.5, "xanchor": "center"}
        else:
            stylesheet = [
                style
                for style in current_stylesheet
                if style["style"].get("border-style") != "double"
            ]
            pie = current_pie
...
def toggle_download_graph_callback(prefix):
    @app.callback(
        ...
        [
            Input(prefix + "_save_graph_open", "n_clicks"),
            Input(prefix + "_save_graph_close", "n_clicks"),
        ],
...
def toggle_group_highlighter_callback(prefix):
    @app.callback(
        ...
        [
            Input(prefix + "_group_highlighter_open", "n_clicks"),
            Input(prefix + "_group_highlighter_close", "n_clicks"),
        ],
...
def get_range_clusters_callback(
    prefix,
    G,
    maj,
    Evals,
    ...
    def get_range_clusters(component, method, coloring, n_custom):
        ...
        if (
            coloring == "custom"
            and n != n_custom
            and dash.callback_context.triggered[0]["prop_id"]
            == prefix + "_coloring_dropdown.value"
        ):
            n = n_custom
...
def custom_clustering_section_callback(
    prefix,
    G,
    Evals,
    Evects,
    ...
    def custom_clustering_section(component, method, n_clusters, clusters):
        ...
        if method == "spectral_group":
            ...
            for n, cl in enumerate(clusters):
                try:
                    clusters_data[cl].append(
                        list(dict(graph.nodes("nameID")).values())[n]
                    )
                except:
                    clusters_data[cl] = [list(dict(graph.nodes("nameID")).values())[n]]
            clusters_data = {
                n: ", ".join(clusters_data[n]) for n in range(len(clusters_data))
            }
            table_header = [html.Thead(html.Tr([html.Th("Cluster"), html.Th("Nodes")]))]
            table_body = []
            for cluster in clusters_data:
                table_body += [
                    html.Tr([html.Td(cluster + 1), html.Td(clusters_data[cluster])])
                ]
            table = dbc.Table(
                table_header + [html.Tbody(table_body)],
                className="table table-hover",
                bordered=True,
                id=prefix + "_custom_clusters_table",
            )
            href = "data:text/csv;charset=utf-8," + quote(
                pd.DataFrame(
                    {
                        "Cluster": list(clusters_data.keys()),
                        "Nodes": list(clusters_data.values()),
                    }
                ).to_csv(sep="\t", index=False, encoding="utf-8")
            )
            clustering_data = pd.DataFrame(
                {"Eigenvalue Number": range(len(evals)), "Eigenvalue": evals}
            )
            figure = px.scatter(
                data_frame=clustering_data,
                x="Eigenvalue Number",
                y="Eigenvalue",
            ...
            table_header = [
                html.Thead(html.Tr([html.Th("Community"), html.Th("Nodes")]))
            ]
            table_body = []
            ...
            href = "data:text/csv;charset=utf-8," + quote(
                pd.DataFrame(
                    {
                        "Cluster": range(len(communities)),
                        "Nodes": [", ".join(comm) for comm in communities],
                    }
                ).to_csv(sep="\t", index=False, encoding="utf-8")
            )
            communities_data = pd.DataFrame(
                {
                    "Number of Communities": list(communities_modularity.values()),
                    "Modularity": list(communities_modularity.keys()),
                }
            )
            figure = px.scatter(
                data_frame=communities_data,
                x="Number of Communities",
                y="Modularity",
                title="Modularity Trend in Different Communities Partitioning",
                template="ggplot2",
            )
        elif method == "greedy_modularity_group":
...
def toggle_view_clusters_callback(prefix):
    @app.callback(
        ...
        [
            Input(prefix + "_view_clusters_open", "n_clicks"),
            Input(prefix + "_view_clusters_close", "n_clicks"),
        ],
    ...
    def fittings(value, ERvalue):
        ...
        plot = go.Figure(
            layout={
                ...
                "xaxis": {"title_text": "Node degree, k", "type": "log"},
                "yaxis": {
                    "title_text": "Normalized Frequency of Nodes with degree k, n(k)",
                    "type": "log",
                },
                "template": "ggplot2",
            }
        ...
        yvalues = np.array([])
        for d, val, title, to_show in [
            [data, value, prefix.replace("_", " ").title(), "power_law"],
            [ERdata, ERvalue, "Erdős-Rényi", "stretched_exponential"],
        ]:
            ...
            for dist in [
                ...
                plot.add_trace(
                    go.Scatter(
                        x=reduced,
                        y=getattr(fitted, dist).pdf(reduced) * scalefactor,
                        mode="lines",
                        name=title + " " + dist.replace("_", " ").title(),
                        visible=visible,
                    )
                )
        ...
        )  # for a transparent background but keeping modebar acceptable colors, "x":1.25
        return plot
...
def toggle_download_interactome_callback(prefix):
    @app.callback(
        ...
        [
            Input(prefix + "_save_interactome_open", "n_clicks"),
            Input(prefix + "_save_interactome_close", "n_clicks"),
        ],
...
def build_callbacks(
    ...
    custom_clustering_section_callback(
        prefix,
        G,
        evals,
        evects,
