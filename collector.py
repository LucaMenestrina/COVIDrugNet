...
class collector:
    ...
    def virus_host_interactome(self):
        ...
        def replace_minor_components(graph, pos, scalefactor=1.25):
            ...
            for comp in components:
                if comp != maj:
                    ...
                    pcomp = nx.rescale_layout_dict(
                        graphviz_layout(
                            graph.subgraph(comp), prog="neato", args="-Goverlap=scalexy"
                        ),
                        1,
                    )
                    for node in comp:
        ...
        targeted_genes = human.intersection(
            set(dict(nx.get_node_attributes(self.__targettarget, "Gene")).values())
        )
        drugs = {}
        ...
        networker_edges = {
            (source, target) for source, targets in drugs.items() for target in targets
        }
        ...
        nx.set_node_attributes(G, {node: node for node in G.nodes()}, "Gene")
        nx.set_node_attributes(
            G, {node: True if node in viral else False for node in G.nodes()}, "Viral"
        )
        nx.set_node_attributes(
            G, {node: True if node in human else False for node in G.nodes()}, "Human"
        )
        nx.set_node_attributes(
            G,
            {node: True if node in targeted_genes else False for node in G.nodes()},
            "Targeted",
        )
        nx.set_node_attributes(
            G, {node: True if node in drugs else False for node in G.nodes()}, "Drug"
        )
        ...
        self.save_graph(
            ...
            pd.DataFrame({"Source": e[0], "Target": e[1]} for e in edges),
            G,
