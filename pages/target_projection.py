...
neighbors = set(dt.neighbors("Fostamatinib (DB12010)")).difference(
    set(dt.neighbors("Artenimol (DB11638)"))
)
edges_to_show = [
    edge
    for edge in edges
    if not (edge["data"]["source"] in neighbors and edge["data"]["target"] in neighbors)
]
                    ...
                    [
                        html.Center(
                            [
                                ...
                                html.Br(),
                                html.P(
                                    "In order to improve the user experience, we decided to hide some of the edges connecting the neighbors of Fostamatinib and Artenimol (%d),"
                                    % (len(edges) - len(edges_to_show)),
                                    style={"font-size": "110%"},
                                ),
                                html.Br(),
                                # html.P("most of them would be hidden behind the two jumbles induced by these two drugs anyway", style={"font-size":"110%"}),
                                html.P(
                                    "! This will only affect the visualization, analyses will not be influenced !",
                                    style={"font-size": "133%"},
                                ),
                                html.Br(),
                                html.Br(),  # H
                                html.P(
                                    "It is possible to override this choice here:",
                                    style={"font-size": "110%"},
                                ),
                                daq.BooleanSwitch(
                                    on=False,
                                    label="Show All Edges",
                                    id=prefix + "_show_all_edges_modal",
                                ),
                                html.Br(),
                                html.P(
                                    [
                                ...
                                html.Br(),  # H
                                html.P(
                                    "Please wait a few seconds while the page loads..."
                                ),
                                dcc.Interval(
                                    id=prefix + "_progress_interval",
                                    n_intervals=0,
                                    interval=500,
                                    max_intervals=3,
                                ),
...
build_callbacks(
    prefix, G, nodes, edges, edges_to_show, *common_data_generator(prefix, G)
)
